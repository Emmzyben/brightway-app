...
import useSearchProvidersByName from '../../hooks/useSearchProvidersByName';

const { width } = Dimensions.get('window');

const SearchCategoryDetailScreen = ({ navigation, route }) => {
    const { name: ProviderName } = route.params; 
    const { providers, loading, error } = useSearchProvidersByName(ProviderName); 
    ...
    function nearestDoctorsInfo() {
        ...
        const renderItem = ({ item }) => (
            <TouchableOpacity
                activeOpacity={0.8}
                onPress={() => navigation.push('DoctorDetail', { email: item.email, username: item.username })} // Pass both email and username
    ...
    function header() {
        return (
            <View style={styles.headerWrapStyle}>
                ...
                <Text>Search results for {ProviderName}</Text>
                <MaterialIcons
                    size={24}
...
export default SearchCategoryDetailScreen;
